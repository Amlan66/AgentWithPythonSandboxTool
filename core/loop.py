    ...
    async def run(self):
        ...
        for step in range(max_steps):
            ...
            while lifelines_left >= 0:
                ...
                # === Planning ===
                # Allow planning even with no tools - agent might synthesize from existing data
                if selected_tools:
                    tool_descriptions = summarize_tools(selected_tools)
                else:
                    # No tools selected - agent should synthesize from provided context
                    tool_descriptions = "No tools available. Synthesize answer from the information already provided in the user input."
                    ...
                    if isinstance(result, str):
                        ...
                        if result.startswith("FINAL_ANSWER:"):
                            ...
                            self.context.user_input_override  = (
                                ...
                                f"🚨 IMPORTANT: Your last tool call already fetched this information:\n\n"
                                f"{content}\n\n"
                                f"DO NOT call the same tool again to fetch this information.\n"
                                f"DO NOT call any search or fetch tools - you already have the data above.\n\n"
                                f"ANALYZE the information above and return:\n"
                                f"FINAL_ANSWER: your synthesized answer based on the data above\n\n"
                                f"Only call a different tool if you need DIFFERENT information not present above."
                            )
