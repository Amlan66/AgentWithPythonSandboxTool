...
async def main():
    ...
    from modules.heuristics import get_validator, reset_validator
    ...
    try:
        while True:
            ...
            if user_input.lower() == 'new':
                current_session = None
                reset_validator()  # Reset validator for new session
                continue
            
            # === HEURISTIC VALIDATION: Input Validation ===
            validator = get_validator()
            
            # 1. Check input length
            is_valid, error_msg = validator.validate_input_length(user_input)
            if not is_valid:
                print(f"❌ Input validation failed: {error_msg}")
                continue
            
            # 2. Check for suspicious characters
            is_valid, error_msg = validator.validate_ascii_content(user_input, strict=False)
            if not is_valid:
                print(f"⚠️  Warning: {error_msg}")
                # Continue anyway but log the warning
            
            # 3. Check for API key exposure
            is_valid, error_msg = validator.validate_api_key_exposure(user_input)
            if not is_valid:
                print(f"🚫 Security issue: {error_msg}")
                continue
